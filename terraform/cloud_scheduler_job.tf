resource "google_cloud_scheduler_job" "update_assigned_issues_database_trigger" {
  name             = "update-assigned-issues-database-trigger"
  region           = var.location
  description      = "Invoke a Cloud Run container on a schedule for updating assigned issues in notion."
  schedule         = "0 * * * *"
